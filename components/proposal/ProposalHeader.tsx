...
export default function ProposalHeader({ proposta, config, expirada }: ProposalHeaderProps) {
    ...
    return (
        <>
            <div className="bg-white rounded-none shadow-swiss border-2 border-black p-8 mb-6 relative overflow-hidden">
                {/* Background Decoration (Sharp) */}
                <div className="absolute top-0 right-0 w-64 h-64 bg-brand-primary-50 rounded-none -translate-y-1/2 translate-x-1/2 opacity-60 pointer-events-none"></div>
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-6 relative z-10">
                    ...
                    <div className="flex flex-col items-center md:items-end">
                        ...
                        {!expirada && diasRest <= 5 && (
                            <div className="mt-2 flex items-center gap-1.5 px-3 py-1 bg-orange-50 text-orange-700 rounded-none text-xs font-bold border border-orange-100">
                        ...
                        {expirada && (
                            <div className="mt-2 flex items-center gap-1.5 px-3 py-1 bg-red-50 text-red-700 rounded-none text-xs font-bold border border-red-100">
                                Expirada
                            </div>
                        )}
                    </div>
            ...
            {config?.textosProposta?.introducao && (
                <div className="bg-white rounded-none shadow-swiss border-2 border-black mb-8 animate-fade-in-up delay-100 overflow-hidden">
                    <div className="p-8">
                        <div className="flex items-center gap-3 mb-4">
                            <div className="w-10 h-10 rounded-none bg-brand-primary-50 flex items-center justify-center text-brand-primary-500 border border-brand-primary-100">
...
function TrustItem({ icon, title, subtitle }: { icon: React.ReactNode, title: string, subtitle: string }) {
    return (
        <div className="p-6 flex flex-col items-center text-center md:flex-row md:text-left gap-4 hover:bg-brand-primary-50/50 transition-colors duration-300 group">
            <div className="w-10 h-10 rounded-none bg-white flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform duration-300 border border-gray-200 shadow-sm text-brand-primary-500">
                {icon}
            </div>
            <div>
